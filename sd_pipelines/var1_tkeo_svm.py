...
def refit_svm_and_predict(raws_list, configs, features_configs):
    with suppress_stdout():
        # preprocessing
        raws_list_mod = common_preprocessing_pipeline(raws_list, sfreq=configs['sfreq'], add_tkeo=True,
    ...
    del epochs_list
    # load model
    model = joblib.load(os.path.join(os.getcwd(), "", 'sd_pipelines', "", "models", "", 'var1_tkeo_svm.joblib'))
    # partial fit
...
def predict(raws_list, configs, features_configs):
    with suppress_stdout():
        raws_list_mod = common_preprocessing_pipeline(raws_list, sfreq=configs['sfreq'],
                                                      bp_filter=configs['bp'], add_tkeo=True, n_jobs=configs['n_jobs'])
        # get_epochs
        epochs_list = get_epochs(raws_list_mod, configs['epoch_hs'], configs['overlap'])
    del raws_list_mod
    # compute features
